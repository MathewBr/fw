<div class="nav-tabs-custom">
    <ul class="nav nav-tabs">
        <button class="btn btn-danger btn-xs delete" onclick="return false;">Очистить</button>
                ...
                <div class="tab-pane<?php if($i == 1) echo ' active' ?>" id="tab_<?= $group_id ?>">
                    <?php foreach($this->attrs[$group_id] as $attr_id => $value): ?>
                        ...
                        <div class="form-group">
                            <label>
                                <input class="filters-checkbox" type="checkbox" name="attrs[<?= $group_id ?>]" value="<?= $attr_id ?>"<?= $checked ?>> <?= $value ?>
                            </label>
                        </div>
                        <?php $i++; endforeach; ?>
                </div>
