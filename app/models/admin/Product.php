...
class Product extends AppModel{
    ...
    public function uploadImg($name, $wmax, $hmax){//$name or single, or multi
        $uploaddir = WWW . '/images/';
        $ext = strtolower(preg_replace("#.+\.([a-z]+)$#i", "$1", $_FILES[$name]['name']));//mime img
        $types = array("image/gif", "image/png", "image/jpeg", "image/pjpeg", "image/x-png");// allowed values
        ...
        if (@move_uploaded_file($_FILES[$name]['tmp_name'], $uploadfile)){
            ...
            self::resize($uploadfile, $uploadfile, $wmax, $hmax, $ext);//in this case, just overwrite the picture in the same place
